...
$(window).on('load', function () {
    ...
    for (let i = 0; i < cnt; i++) {
        ...
        $('#schedule_date').append(
            '<div class="rect rects' + i + ' flex_cont" onclick="date_click(this)">'
            + '<input value="movie_' + i + '" hidden="true">'
            + '<div class="date_box">'
            + '<h1 class="date">' + date + '</h1>'
            + '<h3 class="date_wd">' + '(' + wd[w] + ')' + '</h3>'
            + '<input hidden="true" value="' + hidden_date + '">'
            + '</div>'
            + '</div>'
        );
        w++;
